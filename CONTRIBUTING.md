...
4.  Realize o **Commit** seguindo as convenções (veja seção 5).
...
2.  **Nunca edite o `requirements.txt` manualmente.** Ele é um artefato gerado.
3.  Se você alterou as dependências, **você deve atualizar o arquivo de requisitos** para que o Docker funcione:
    ```bash
...
2.  **Treino:** O modelo treina e salva o `.pth`?
    ...
    ```
3.  **Docker:** A imagem constrói sem erros? (Isso valida o `requirements.txt`)
    ```bash
